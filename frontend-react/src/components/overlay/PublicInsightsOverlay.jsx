...
function PublicDataSheetView({ folder, sheet }) {
  ...
  const viewRows = useMemo(() => {
   ...
   // 🔒 Masquer run_id / user_id sur la feuille Config (public only)
  const isConfigSheet = /^(config)$/i.test(String(sheet || ""));
  const keyOf = (row) => {
    // clé affichée dans la 1ʳᵉ colonne type "Metric"/"Clé"/"Paramètre"…
    const h0 = headers?.[0];
  ...
  return (
    ...
    {/* Toolbar publique : CTA toujours visible ; select affiché seulement sur "Par_Heure" */}
      <div className={`pub-toolbar ${/par[_ ]?heure/i.test(String(sheet || "")) ? "has-left" : ""}`}>
        ...
        {/* CTA toujours à droite */}
        <CTAButton href="/login" className="tz-cta">
          <span className="t-long">Tester BackTradz gratuitement</span>
          <span className="t-short">Tester</span>
        </CTAButton>
      </div>
