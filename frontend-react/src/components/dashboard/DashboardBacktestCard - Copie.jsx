...
export default function DashboardBacktestCard({ bt = {}, onDeleted }) {
  ...
  const downloadHref = xlsx ? downloadXlsxUrl(xlsx) : null;
  ...
  <>
    <div className="dbt-card">
      ...
      {/* Actions */}
      <div className="dbt-actions-primary">
        {downloadHref ? (
          <CTAButton href={downloadHref} leftIcon="⬇️" fullWidth>
            Télécharger (.xlsx)
          </CTAButton>
        ) : (
      ...
      {/* Secondaires : on ajoute le bouton Insights à côté de Détails/Supprimer */}
        {/* Secondaires : on ajoute le bouton Insights à côté de Détails/Supprimer */}
          <div className="dbt-actions-secondary">
              {/* 👉 Détails ouvre désormais l’overlay Insights */}
              <DetailButton fullWidth onClick={() => setOpenInsights(true)}>Détails</DetailButton>
              <DeleteButton fullWidth onClick={() => setAsk(true)}>Supprimer</DeleteButton>
            </div>
