...
def launch_backtest(req: BacktestRequest, authorization: str = Header(None, alias="X-API-Key")):
    ...
    try:
        ...
        # 🏷 Récupérer run_id depuis le JSON de config écrit par runner_core
        run_id_value = None
        try:
            out_dir = Path(analysis_xlsx_path).parent if analysis_xlsx_path else None
            if out_dir and out_dir.exists():
                for jf in out_dir.glob("*.json"):
                    with open(jf, "r", encoding="utf-8") as f:
                        j = json.load(f)
                    if isinstance(j, dict) and j.get("run_id"):
                        run_id_value = j["run_id"]
                        break
        except Exception as e:
            print("⚠️ Impossible de lire run_id depuis config:", e)
        ...
        try:
            ...
            charge_2_credits_for_backtest(user.id, {
                ...
                "run_id": run_id_value,   # 🆕 identifiant unique du run
            })
        ...
        return {
            ...
            "run_id": run_id_value,   # 🆕 renvoyé au front pour key de carte
        }
