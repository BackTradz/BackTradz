...
async def stripe_webhook(request: Request):
    ...
    try:
        ...
        if evt_type == "checkout.session.completed":
            ...
            if mode == "payment":
                ...
                if paid_eur is None:
                    ...
                    if getattr(user, "has_discount", False) and offer.get("type") in ["one_shot", "credit"]:
                        base = round(base * 0.9, 2)
                    paid_eur = base
                ...
                try:
                    ...
                    create_invoice(
                        ...
                        method="Stripe",
                        transaction_id=session.get("payment_intent") or session.get("id") or "stripe-session",
                        amount=paid_eur,
                        currency="EUR",
                        items=items,
                        created_at=None,
            ...
            # ---------------------------
                ...
                if ok:
                    logger.debug(f"📌 Subscription activée sans crédits (user={user.id}, sub={stripe_subscription_id}).")
                else:
        ...
        if evt_type == "invoice.payment_failed":
            ...
            # 5) 📧 Email d’échec — une seule fois
            try:
                u = users[user_id]
                sub = u.setdefault("subscription", {})
                if not sub.get("failed_mail_sent"):
                    ...
                    if user_email:
                        ...
                        USERS_FILE.write_text(json.dumps(users, indent=2), encoding="utf-8")
                        logger.infos("📧 Email d'échec de renouvellement envoyé (unique).")
            except Exception as _e:
        ...
        logger.exception("❌ Erreur Stripe webhook")
